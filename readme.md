...
taglet is a zero-dependency utility for generating code. It _mostly_ exists to
improve the ergonomics of native ECMAScript [tagged templates][ecmascript]. It
can also be used to generate _partially_ abstract code which is useful when the
same code has multiple target formats (such as TypeScript and JavaScript).
...
has special accommodations for multi-line templates and returns
...
// render as-is:
console.log(line.toString()); // items[1] = 2;

// transform numbers:
const newLine = line.transform((arg) => (typeof arg === "number" ? arg * 2 : arg));
console.log(newLine.toString()); // items[2] = 4;

// supply substitute arguments at render-time:
console.log(line.toString(8, '"hippo"')); // items[8] = "hippo";
```
...
console.log(line.toString()); // const regexp = /\//;
```
...
```javascript
// Literal block template with inferred indention and default chomping
const block = taglet`|
  line one
  line two
`;

console.log(block.toString()); // "line one\nline two\n"
```
...
console.log(block.toString()); // "part one part two\npart three\n"
```

```javascript
// Literal block template with escaped line ending
const block = taglet`|
  line one\
  -thousand
`;

console.log(block.toString()); // "line one-thousand\n"
```
...
```javascript
// Semantic indention in block template
const block = taglet`|
  line one
    line two
`;
...
```javascript
// Indention indicator for block template
...
console.log(block1.toString() === block2.toString()); // true
```
